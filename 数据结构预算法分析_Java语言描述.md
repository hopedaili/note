...
**贪婪算法**（greedy algorithm）分阶段工作。在每一个阶段，可以认为所做决定是最好的，而不考虑将来的后果。通常，这意味着选择的时某个局部最优。这种“眼下能够拿到的就拿”的策略时这类算法名称的来源。贪婪算法不是总能够成功的。当算法终止时，我们希望局部最优等于全局最优。如果是这样的话，那么算法就是正确的；否则算法得到的是一个**次最优解**（suboptimal solution）。如果不要求绝对最佳答案，有时使用简单的贪婪算法生成近似的答案，而不是使用通常产生准确答案的复杂算法。

### 10.1.1 一个简单的调度问题

这个这一节假设**非预占调度**（nonpreemptive scheduling）：一旦开始一个作业，就必须把该作业运行到完成。
...
在这一节，考虑贪婪算法的第二个应用，文件压缩（file compression）。

一种简单的策略就是让代码的长度从字符到字符是变化不等的，同事保证经常出现的字符其代码要短。注意，如果所有字符都以相同的频率出现，那么结成的问题不可能存在。

#### 哈夫曼算法

假设字符个数为 C。哈夫曼算法（Huffman‘s algorithm）可以描述如下：算法对由树组成的一个森林进行。一棵树的权等于他的树叶的频率的和。任意选取最小权的两棵树 $T_1$ 和 $T_2$，并任意形成以 $T_1$ 和 $T_2$ 为子树的新树，将这样的过程进行 C - 1 次。在算法的开始， 存在 C 棵单节点树——每个字符一棵。在算法结束时得到一棵树，这棵树就是最优哈夫曼编码树。

概述哈夫曼算法产生最优代码证明思路：

首先，反证法不难证明树必然是满树。其次，反证法容易证明两个频率最小的字符 α 和 β 必然是两个最深的节点（虽然其他节点可以同样地深）。然后可以论证，在相同深度上任意两个节点处的字符可以交换而不影响最优性。这说明，总可以找到一颗最优树，含有两个最不经常出现的符号作为兄弟。

如果依权排序将这些树保存在一个优先队列中，在绝不会超过 C 个元素的优先队列上将进行一次 buildHeap，2C - C次 deleteMin，和 C - 2 次 insert，因此运行时间为 O（C log C）。若使用一个链表简单实现该队列，则给出一个 O（$C^2$）算法。在 ASCII 字符集的典型情况下，C 足够小，这使得二次运行时间是可以接受的。在这样的应用中，实际上所有运行时间都将花费在读取输入文件和写入压缩文件所需要的磁盘 I/O 上。

有两个细节必须要考虑。首先，压缩文件的开头必须要传送编码信息，否则将不可能译码。对于一些小文件，传送编码信息表的代价将超过压缩中任何可能的节省，最后的结果可能是文件扩大。当然，这可以检测到且原文件可原样保留。对于大型文件，信息表的大小是无关紧要的。

第二个问题正如所描述的，该算法是一个两趟扫描算法。第一趟搜集频率数据，第二趟进行编码。显然，对于处理大型文件的程序来说这个性质不是我们所希望的。

### 10.1.3 近似装箱问题

这一节，我们考虑某些解决**装箱问题**（bin packing problem）的算法。这些算法将运行的很快，但未必产生最优解。可以证明所产生的解据最优解不太远。

有两种版本的装箱问题。第一种时**联机装箱问题**（on-line bin packing problem）。在这种问题中，每一件物品必须放入一个箱子后才能处理下一件物品。第二种是**脱机装箱问题**（off-line bin packing problem）。在一个脱机装箱算法中，我们做任何事情都需要等到所有的输入数据全被读取之后才进行。

#### 联机算法

联机算法不总能够给出最优解。联机算法从不知道输入何时会结束，因此它提供的任何性能必须在整个算法的每一时刻成立。

**定理 10.1** 存在使得任意联机装箱算法至少使用 4/3 最有箱子数的输入。

证明：没有联机算法能够保证使用小于 4/3 的最优装箱数完成装箱。

#### 下项适合算法

大概最简单的算法就属**下项适合**（next fit）算法了。当处理任何一项物品时，检查看他是否还能装进刚刚装进物品的同一个箱子中取。如果能够装进入，那就就把它放入该箱中；否则，就开辟一个新的箱子。以线性时间运行。

**定理 10.2** 令 M 时将一列物品 I 装箱所需要的最优装箱数，则下项适合算法所用箱数绝不超过 2M 个箱子。存在一些顺序使得下项适合算法用箱数达 2M - 2 个。

#### 首次适合算法

**首次适合算法**（first fit）的策略时依序扫描这些箱子并把新的一项物品放入能盛下它的第一个箱子中。因此，只有当前面那些放置物品的箱子已经容不下当前物品的时候，才开辟一个新箱子。

**定理 10.3** 令 M 是将一列物品 I 装箱所需要的最优箱子数，则首次适合算法使用的箱子数绝不多于 $\frac{17}{10}M$。存在使得首次适合算法使用 $\frac{17}{10}（M-1）$ 个箱子的序列。

#### 最佳适合算法

**最佳适合**（best fit）装箱法，把一项新物品放入所有箱子中能够容纳他的最满的箱子。最佳适合算法不会超过最优算法的约 1.7 倍。
...
分（divide）：递归解决较小问题（基本情况除外）。

治（conquer）：然后从子问题的解构建 原问题的解。

传统上，在正文中至少含有两个递归调用的例程叫做分治算法，而正文中只含有一个递归嗲用的例程不是分治算法。一般坚持子问题是不相交的（即基本上不重叠）。

### 10.2.1 分治算法的运行时间

**定理 10.6**

![image-20201216151715280](img/数据结构预算法分析_Java语言描述.assets/定理10.6.png)

**定理 10.7 定理10.8**

![image-20201216151958921](img/数据结构预算法分析_Java语言描述.assets/定理10.7-10.8.png)
...
将递归算法重写写成非递归算法，让后者把那些子问题的答案系统地记录在一个表格内。利用这种方法的一种技巧叫做**动态规划**（dynamic programming）。

### 10.3.1 用一个表代替递归

递归算法慢的原因在于算法模拟了递推。冗余计算的增长是爆炸的。

![image-20201217143448966](img/数据结构预算法分析_Java语言描述.assets/跟踪斐波纳契数的递归计算.png)
